...
//			if (number.equals("01") || number.equals("02") || number.equals("08") || number.equals("09"))
    ...
    public static boolean checkMobilePhoneNumberNew(String number) {
        ...
        if (matcher.matches() && (number.length() == 10)) {
            if ("09".equals(number.substring(0, 2)) || Arrays.asList(new String[]{"032", "033", "034", "035", "036", "037", "038", "039", "052", "056", "058", "059", "070", "076", "077", "078", "079", "081", "082", "083", "084", "085", "086", "088", "089", "087"}).contains(number.substring(0, 3)))
                return true;
        }
    ...
    public static List<String> findMatch(String start, String end, String search) {
        List<String> params = new ArrayList<String>();
        if (isNullOrEmpty(search) || isNullOrEmpty(start) || isNullOrEmpty(end)) {
            return params;
        }

        int startIdx = search.indexOf(start);
        while (startIdx > -1) {
            int endIdx = search.indexOf(end, startIdx + 1);
            if (endIdx > -1) {
                String found = search.substring(startIdx + start.length(), endIdx - end.length() + 1);
                params.add(found);
            }
            startIdx = search.indexOf(start, (endIdx > startIdx ? endIdx : startIdx) + 1);
        }

        return params;
    }

    public static String replace(String find, String replace, String search) {
        if (isNullOrEmpty(find) || isNullOrEmpty(replace)) {
            return search;
        }
        ...
        int startIdx = search.indexOf(find);

        if (startIdx > -1) {
            return ((startIdx == 0) ? replace + search.substring(startIdx + find.length(), search.length())
                    : search.substring(0, startIdx)) + replace + search.substring(startIdx + find.length(), search.length());
        } else {
            return search;
        }
    }
    ...
    public static BigDecimal toBigdecimalOrDefaultZero(String s) throws Exception {
        return isNumeric(s) ? toBigDecimal(s) : BigDecimal.ZERO;
    }
}
